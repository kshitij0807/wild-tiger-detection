...
class COCODetection(data.Dataset):
    ...
    def pull_item(self, index):
        ...
        try:
            ...
            bboxes = bboxes / np.array([width, height, width, height], dtype=np.float64)
...
if __name__ == '__main__':
    get_dataloader = partial(DataLoader, batch_size=224, num_workers=0,
    ...
    #dataset = config.pop('train')
    #nd = get_dataset(dataset)
    datasets = map(config.pop, ('train', 'val'))
    
    datasets = map(get_dataset, datasets)
    train, val = map(get_dataloader, datasets)
    #print(nd.pull_annotation(1))
