...
from pytorchcv.models.fdmobilenet import fdmobilenet_wd2 as backbone 
from pytorchcv.models.shufflenetv2b import shufflenetv2b_wd2 as backbone2 #attempted model
from pytorchcv.models.shufflenetv2b import shufflenetv2b_w2 as backbone3
# from pytorchcv.models.efficientnet import efficientnet_b3b as backbone3
# from pytorchcv.models.squeezenet import squeezenet_v1_0 as backbone4

# from pytorchcv.models.efficientnet import efficientnet_b2b as backbone5
from pytorchcv.models.shufflenetv2b import shufflenetv2b_w1 as backbone6
...
class RetinaNet(nn.Module):
    ...
    def __init__(self, config, pretrained=True):
        ...
        self.regression = nn.ModuleList([self._make_head(int(num_anchors*4), x, num_filters_fpn) for x in range(len(feature_maps))])
        ...
        self.classification = nn.ModuleList([self._make_head(int(num_anchors*self.num_classes), x, num_filters_fpn) for x in range(len(feature_maps))])
...
class FPN(nn.Module):
    ...
    def __init__(self, config, num_filters=128, pretrained=True):
        ...
        net = backbone3(pretrained=True).features
        ...
        self.conv5 = DConv2d(in_channels=976, out_channels=num_filters)
        ...
        self.lateral4 = nn.Conv2d(976, num_filters, kernel_size=1, bias=False)
        #self.lateral3 = nn.Conv2d(128, num_filters, kernel_size=1, bias=False)
        self.lateral3 = nn.Conv2d(488, num_filters, kernel_size=1, bias=False)
        #self.lateral2 = nn.Conv2d(64, num_filters, kernel_size=1, bias=False)
        self.lateral2 = nn.Conv2d(244, num_filters, kernel_size=1, bias=False)
    ...
    def forward(self, x):
        ...
        map5 = self.conv5(enc2)
        map6 = self.conv6(map5)
        map7 = self.conv7(map6)

        # Lateral connections
        lateral4 = self.lateral4(enc2)
        lateral3 = self.lateral3(enc1)
        lateral2 = self.lateral2(enc0)
...
def build_retinanet(config, parallel=True):
    ...
    #print(f'model we are going to use has the following architecture{(model)}')
    return model
